...
class ProxyRefreshSchedule(ProxyManager):
    ...
    def valid_proxy(self):
        ...
        raw_proxy_item = str(self.db.pop_proxy(self.raw_proxy_queue))
        while raw_proxy_item:
            if not self.db.is_exists_proxy('useful_proxy', raw_proxy_item) and validate_useful_proxy(raw_proxy_item):
...
def run():
    ...
    blocking_scheduler.add_job(build_refresh_thread, 'interval', minutes=random.randint(5, 15))
